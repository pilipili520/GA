...
void input() {//初始化的输入
	...
	for (int i = 0; i < 4; i++) {
		...
		for (int j = 0; j < totItem; j++) {
			++cot;
			cin >> timetable[cot]>>destable[cot];
		}
	}
}
...
void cal(unit& s) {
	...
	vector<huo> readyOut;
	int  car[3] = { 0,0,0 };//diao是吊车记录时间，car是小车也是记录的时间
	diaoCar diao[3];
	int shiCha = 0;
		...
		for (int i = 1; i <= 2; i++) {//遍历吊车
			if (!boWei.empty()) {//
				...
				if (!diao[i].tim) {//当前吊车空闲
					...
					huo tmp = boWei.front();
					if (!tmp.q.empty()) {
						diao[i].tim = timetable[tmp.q.top().ID];
						diao[i].des = destable[tmp.q.top().ID];
			...
			if (isAllOut&&diao[i].tim) {
				...
				if (!diao[i].tim) { waitForCar.push(diao[i].des);//debug++; 
				}
			}
		}
		//驶离部分
