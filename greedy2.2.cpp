...
int main()
{
	...
	priority_queue<int, vector<int>, greater<int> >  waitForCar;//等待小车运送的货物，就按先来后到TODO
	while (++clk) {//这个是表示1s之后的结果，模拟真的时钟
		...
		for (int i = 0; i < boWei.size(); i++) {//把来的货物都用这个qiDiao的小根堆来维护
			while (!boWei[i].q.empty()) {
				qiDiao.push(boWei[i].q.top().timecost);
				boWei[i].q.pop();
			}
		}
		//起吊部分
		for (int i = 1; i <= 2; i++) {//遍历吊车
			if (!diao[i].tim && !qiDiao.empty()) {
				diao[i].tim = qiDiao.top();
				//cout << diao[i] << endl;
				qiDiao.pop();
				continue;
			}
			if(diao[i].tim) {
				diao[i].tim--;
				if (!diao[1].tim && !diao[2].tim && qiDiao.empty()) {
					for (int i = 0; i < 2; i++) {
